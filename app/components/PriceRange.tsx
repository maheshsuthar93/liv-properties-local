...
const PriceRange = ({
  ...
  return (
    <div ref={ref}>
      <form
        className={`price-popup flex w-[300px] flex-col gap-[14px] bg-[#EDDFD0] px-[20px] py-[25px] xs:w-[320px] ${className}`}
        onSubmit={onSubmit}
        onReset={onReset}
      >
        {!isValidRange && (
        ...
        <div className="relative flex flex-row justify-between gap-[14px] text-[rgb(130,113,97)]">
          <div className="w-[calc(50% - 7px)] flex flex-col gap-[10px] text-left">
            ...
            <select
              ...
              className={`sm:leading-1 block w-full border-0 border-b-[1px] border-[#827161] bg-transparent py-1.5 !pr-[30px]
                        pl-0 text-left text-[100%] text-[rgb(130,113,97)] placeholder-[#eddfd0] ring-0 ring-inset ring-transparent
                        transition duration-200 ease-in-out hover:ring-[#EDDFD0]/50 focus:border-[#827161]  focus:outline-none focus:ring-0 focus:ring-inset focus:ring-[#EDDFD0]`}
            >
              <option className="border-[#eddfd0]" value="">
                {/* Min. Price (AED) */}
                {minPlaceholder ?? 'Min. Price (AED)'}
              </option>
              {generateOptions('min')}
            </select>
            {/* <input
